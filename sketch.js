...
function setup() {
  ...
  // Load Elon image using GitHub Pages path
  loadImage(
    '/doge-game/assets/elon.png',  // Updated path for GitHub Pages
    // Success callback
    img => {
...
function draw() {
  ...
  if (state === 'showingElon' && showingElon) {
    ...
    if (isImageLoaded && elonImage) {
      ...
      // Center Elon
      imageMode(CENTER);
      let elonSize = 400; // Larger size for center display
      ...
      // Draw Elon centered
      image(elonImage, width/2, height/2, elonSize, elonSize);
      pop();
    }
...
function drawFlashingOutcome() {
  ...
  // Make the panel wider and taller to fit all text
  let panelWidth = 600;  // Increased from 500
  let panelHeight = 140; // Increased from 120
  ...
  textSize(36);
  text(outcome.label, centerX, centerY - 25);
  
  // Amount/multiplier
  textSize(32);
  if (outcome.type === 'gain') {
    text(`+${formatMoney(outcome.amount)}`, centerX, centerY + 35);
  } else if (outcome.type === 'double') {
    text(`${outcome.multiplier}X MULTIPLIER!`, centerX, centerY + 35);
  } else {
    fill(255, 0, 0);
    text('SAVINGS LOST!', centerX, centerY + 35);
  }
  pop();
}
...
function shareOnX() {
  ...
  const url = 'https://tinyurl.com/doge-game';  
